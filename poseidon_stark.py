...
def poseidon_full_state(scalar_val, steps):
    """Apply Poseidon permutation 'steps' times, starting with scalar in first state element.
    ...
    for i in range(steps):
        ...
        if is_full:
            ...
            # Apply round constants to ALL elements (Poseidon difference)
            for j in range(16):
                state[j] = (state[j] + ROUND_CONSTANTS[rc_idx]) % modulus
                rc_idx += 1
            # Apply S-box only to first element
            state[0] = pow(state[0], DEFAULT_ALPHA, modulus)
            # Use MATRIX_FULL for both full and partial rounds (Poseidon difference)
            state = mat_vec_mul(MATRIX_FULL, state, modulus)
...
def mk_poseidon_proof(inp, steps):
    ...
    for i in range(steps):
        ...
        for j in range(16):
            # In Poseidon, round constants always apply to all elements
    ...
    print(f"[mk_poseidon_proof] Traces generated, starting FFT on s_full_trace and rc_traces...")
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        print(f"[mk_poseidon_proof] Waiting for s_full inverse FFT...")
        s_full_poly = s_inv_future.result()
        print(f"[mk_poseidon_proof] Got s_full_poly, waiting for rc_polys...")
        rc_polys = [f.result() for f in rc_inv_futures]
        print(f"[mk_poseidon_proof] Got all rc_polys")
    
    # Low-degree extension using FFT (fast O(n log n) instead of O(n^2))
    # Pad polynomial coefficients with zeros to precision, then FFT
    print(f"[mk_poseidon_proof] Computing s_full_evals forward FFT...")
    s_full_evals = s_full_poly + [0] * (precision - len(s_full_poly))
    s_full_evals = fft(s_full_evals, modulus, G2, inv=False)
    
    print(f"[mk_poseidon_proof] Computing rc_evals forward FFTs...")
    rc_evals = []
    for j in range(16):
        rc_padded = rc_polys[j] + [0] * (precision - len(rc_polys[j]))
        rc_evals.append(fft(rc_padded, modulus, G2, inv=False))
    ...
    # step i corresponds to the state after applying Poseidon permutation i times
    computational_trace = []
    ...
    for i in range(steps - 1):  # Apply permutation steps-1 times to get steps entries total
        ...
        if is_full:
            ...
            # Poseidon partial round: apply S-box only to first element
            current_state[0] = pow(current_state[0], DEFAULT_ALPHA, modulus)
            # Use MATRIX_FULL for both (Poseidon difference)
            current_state = [sum(MATRIX_FULL[r][c] * current_state[c] for c in range(16)) % modulus for r in range(16)]
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for j in range(16):
            ...
            p_fwd_futures.append(executor.submit(fft, p_padded, modulus, G2, inv=False))
    ...
    for i in range(precision):
        ...
        # Poseidon: only first element gets S-box in partial rounds
        sbox_state = [pow(state_plus_rc[0], DEFAULT_ALPHA, modulus)] + [
            (pow(state_plus_rc[k], DEFAULT_ALPHA, modulus) * s_f + state_plus_rc[k] * (1 - s_f)) % modulus for k in range(1, 16)]
        for j in range(16):
            # Poseidon uses MATRIX_FULL for both partial and full rounds
            mat_full_res = sum(MATRIX_FULL[j][k] * sbox_state[k] for k in range(16)) % modulus
            
            p_next_calculated = mat_full_res
    ...
    o = [mtree[1],
         ...
         prove_low_degree(l_evaluations, G2, steps * 3, modulus, exclude_multiples_of=extension_factor)]
...
def verify_poseidon_proof(inp, steps, output, proof):
    ...
    # First verify that the output matches the actual Poseidon computation
    ...
    verify_low_degree_proof(l_root, G2, fri_proof, steps * 3, modulus, exclude_multiples_of=extension_factor)
