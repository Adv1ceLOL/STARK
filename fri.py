...
def _merkelize_padded(values):
    ...
    if padded_n != n:
        ...
        if isinstance(values[0], int):
            ...
            padded = list(values) + [b'\x00' * 32] * (padded_n - n)
...
def prove_low_degree(values, root_of_unity, maxdeg_plus_1, modulus, exclude_multiples_of=0):
    ...
    # With mixed-radix steps (2^k × 127), we hit 127 or 254 which aren't divisible by 4
    if maxdeg_plus_1 <= 16 or len(values) % 4 != 0:
...
def verify_low_degree_proof(merkle_root, root_of_unity, proof, maxdeg_plus_1, modulus, exclude_multiples_of=0):
    ...
    # Use iterative multiplication to find the order (works for any order,
    # not just powers of 2 — the old squaring approach infinite-loops on
    # non-power-of-2 orders like 508 = 4 × 127)
    testval = root_of_unity
    roudeg = 1
    while testval != 1:
        roudeg += 1
        testval = (testval * root_of_unity) % modulus
    ...
    print('Verifying degree <= %d' % maxdeg_plus_1)
    # Accept base case when small enough OR when domain can't be quartic-folded
    assert maxdeg_plus_1 <= 16 or len(data) % 4 != 0, \
